...
_ANIMATED = {
    ...
    "🎯": "5444987348334965906", "💳": "5447453226498552490", "🛒": "5447319442562251569", 
    "🔗": "5447479640547428304"
}
...
"""
                    # Here is the magical premium emoji wrap!
                    premium_live_msg = premium_emoji(raw_live_msg)
                    
                    try:
                        await asyncio.sleep(1) 
                        # using message.get_bot().send_animation for 100% guarantee delivery
                        await message.get_bot().send_animation(
                            chat_id=message.chat_id,
                            animation=gif_url,
                            caption=premium_live_msg,
                            parse_mode="HTML"
                        )
                    except Exception as e1:
                        try:
                            # Fallback 1: Plain emojis with GIF
                            await message.get_bot().send_animation(
                                chat_id=message.chat_id,
                                animation=gif_url,
                                caption=raw_live_msg,
                                parse_mode="HTML"
                            )
                        except Exception as e2:
                            # Fallback 2: Invisible link with premium emojis
                            try:
